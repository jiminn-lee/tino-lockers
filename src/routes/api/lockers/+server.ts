...
export async function GET({ request }: RequestEvent) {
	try {
		...
		const singleLockersResult = await db.select().from(singleLockers).orderBy(singleLockers.id);

		const partnerLockersResult = await db.select().from(partnerLockers).orderBy(partnerLockers.id);
